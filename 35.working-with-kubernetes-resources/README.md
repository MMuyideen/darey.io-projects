...
#### Multiline Strings
Multiline strings can be represented using the `I` or `>` characters:
```yaml
description: |
  This is a multiline
  string in YAML.
...
![Create Deployment](./images/01.create-deployment.png)
...
```
![Minikube svc](./images/02.minikube-svc.png)

![svc running in browser](./images/03.svc-browser.png)

### Working With YAML Files

Let's recall our [docker foundations project](https://github.com/MMuyideen/darey.io-projects/tree/master/28.intro-to-docker) when we pushed an image we built to docker hub. Now let reuse our image in our yaml script for deployment.
...
- **image: dareyregistry/my-nginx:1.0**: Specifies the Docker image to be used for the Nginx container. The image is `ridwanaz/my-nginx` with version `1.O.`
...
```
![Run Deployment](./images/04.create-deployment.png)
...
```
![Get Deployment](./images/05.get-svc.png)
...
```
![Minikube svc](./images/06.minikube-svc.png)
![Access App](./images/07.app-access.png)


## LEARNING SUMMARY

- Key takeaways:
  - Deployments declare desired state and manage replicas; Services provide stable access (ClusterIP / NodePort / LoadBalancer).
  - Pods are the smallest deployable unit; containers run inside Pods and share network/storage.
  - Minikube is a single-node local cluster for development; kubectl is the primary CLI for inspection and management.

- Hands-on skills:
  - Create/apply YAML manifests: `kubectl apply -f <file>`.
  - Expose and access apps: `kubectl expose`, `minikube service`, `minikube tunnel`.
  - Inspect and troubleshoot: `kubectl get`, `kubectl describe`, `kubectl logs`, `kubectl exec`, `kubectl port-forward`.
