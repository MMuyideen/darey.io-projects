...
- name: Automate user creation
  ...
  tasks:
    - name: Create users with additional settings
      user:
        name: "{{ item.username }}"
        state: present
        shell: /bin/bash
...
```
![Run playbook](./images/04.run-playbook.png)
...
ls /home
```
![verify users](./images/05.verify-users.png)
...
```
![ssh access](./images/06.ssh-access.png)
