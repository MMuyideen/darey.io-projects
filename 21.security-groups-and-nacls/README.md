...
## Part - 1
We will use the website we setup in the [Cloud Computing Project](../14.cloud-computing-introduction/README.md).

![EC2 Instance](./images/01.ec2.png)

Here is the security group configuration for the instance. in the inbound rules, only IPv4 SSH traffic on port 22 is permitted to access the instance

![inbound SSH](./images/02.inbound-ssh.png)
...
![Outbound all ](./images/03.outbound-all.png)
...
If you enter `https://3.101.116.20/` into your  browser, and hit enter, you'll notice that the page doesn't load; it keeps attempting to connect. And finally it'll show this page. After some time, you'll likely see a page indicating that the site can't be reached.

![Site Unreachable ](./images/04.site-unreachable.png)
...
2. Please provide a name and description for the new security group.
    ...
    ![Create SG ](./images/05.create-sg.png)
...
3. Now navigate to the instance section of left side bar.
    ...
    ![EC2 ](./images/06.instance-action.png)

4. Choose the Security Group you created
    - Click Add Security group
    - Click on Save

    ![Add SG ](./images/07.add-sg.png)

5. Now it is being attached successfully,
    - If you again copy the public IP address,
    - And try to access the Ip again in the browser, We'll be able to see the data of our website.

    ![Site accessible ](./images/08.site-accessible.png)
...
![Remove Outbound](./images/09.remove-outbound.png)
...
7. You will be able to delete the inbound rule in the same way we have deleted the outbound rule.'
    ...
    ![Removed SG ](./images/10.no-sg-rule.png)
...
So you can't access the website now as shown below
...
> Note we need to add an ssh inbound rule to enable us test the outbound connectivity inside the instance itself

Before adding the Outbound rule, we will ssh into the instance and try to use `curl` command to reach amazon website `curl http://amazon.com`

![Outbound no access ](./images/12.outbound-notworking.png)

Now we add the outbound rule to allow the instance reach the internet
...
8. Click on edit outbound rule in the outbound tab,
    ...
    ![Add http outbound ](./images/11.add-hhtp-outbound.png)

To test the outbound access, we can ssh into the instance and try the curl command again. Now we should get feedback from the website as shown below

![Outbound access ](./images/13.outbound-working.png)
...
2. Navigate to the Network ACLs in the left sidebar.
    ...
    ![Create NACL ](./images/14.create-nacl.png)
...
3. Now, provide a name for your Network ACL,
    ...
    ![Create NACL ](./images/15.create-nacl.png)
...
Similarly, if you look at the outbound rules, you'll observe that it's denying all !

[NACL default](./images/16.default-nacl.png)
...
![Add Nacl inbound ](./images/17.add-rule.png)

Currently, this NACL is not associated with any of the subnets in the VPC.

![NACL no assocition ](./images/18.nacl-no-association.png)
...
[NACL Association ](./images/19.nacl-subnet-ass.png)

You have successfully associated your public subnet to this NACL.

As soon as you have attached this NACL to your public subnet, and then you try to access the website again by typing the URL http://54.255.228.191Z, you will notice that you are unable to see the website.
...
9. If we allow outbound traffic as well,
    ...
    ![EC2 ](./images/21.nacl-add-outbound.png)
...
**NACL**,
Let's remove it so by default it be denied all traffic. Additionally, the outbound rule will be removed, defaulting to deny all traffic by default.
...
Once you're inside the building, there's another layer of security at each room's door. These are like the Security Groups. Each room has its own rules, like "only employees allowed" or "no pets." These rules are specific to each room, just like 

Security Groups are specific to each EC2 instance.
...
- NACL allows all inbound and outbound traffic, Security Group allows all inbound and outbound traffic: Outcome: Website access will be allowed, as both
NACL and Security Group allow all traffic.
